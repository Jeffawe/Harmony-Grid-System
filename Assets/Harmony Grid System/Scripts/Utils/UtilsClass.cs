...
namespace HarmonyGridSystem.Utils
{
    ...
    {
        private static Dictionary<string, int> constraintGroupMap = new Dictionary<string, int>();
        private static int nextGroupId = 1;

        public static int GetOrCreateGroupId(string groupName)
        {
            if (string.IsNullOrEmpty(groupName)) return -1;

            if (!constraintGroupMap.TryGetValue(groupName.ToLower(), out int groupId))
            {
                groupId = nextGroupId++;
                constraintGroupMap[groupName.ToLower()] = groupId;
            }
